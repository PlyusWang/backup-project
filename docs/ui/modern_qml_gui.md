...
- **左栏：文件预览**。列出源目录条目、类型、大小，以及应用当前规则后的归属
  （进入归档 / 被规则排除 / 目录被排除 / 特殊文件）。预览由后台线程扫描，
  最多 300 项，超过会提示“仅预览前 300 项”。
...
共用同一个 Filter。面板不直接访问全局上下文属性，规则模型由 OperationPage
显式注入（ruleModel: filterRuleModel），面板根读一次 model 后同步到本地属性，
子项只绑本地属性 —— 这样避免了 QML 创建顺序导致的运行期空引用告警。
